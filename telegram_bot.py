...
def send_tistory_copy(title: str, content: str, tags: list, summary: str = "", image_prompt_en: str = "") -> None:
...
def get_events(offset: int):
    ...
    for update in updates:
        ...
        if text.startswith("/count"):
            parts = text.split()
            if len(parts) == 3 and parts[1] in TRACKS and parts[2].isdigit():
                value = int(parts[2])
                if 1 <= value <= 20:
                    events.append({"type": "count", "track": parts[1], "value": value})
